...
If a required variable has not been set, you can skip or fail using Jinja2's <span class="title-ref">defined</span> test. For example:
...
This is especially useful in combination with the conditional import of `vars` files (see below). As the examples show, you do not need to use <span class="title-ref">{{ }}</span> to use variables inside conditionals, as these are already implied.
...
If you need to skip the whole task when the loop variable is undefined, use the <span class="title-ref">\|default</span> filter to provide an empty iterator. For example, when looping over a list:
...
The major version of the operating system. For example, the value is <span class="title-ref">16</span> for Ubuntu 16.04.
