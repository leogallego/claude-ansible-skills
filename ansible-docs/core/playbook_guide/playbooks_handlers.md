...
3.  Handlers from roles statically imported via `import_role` tasks.
4.  Handlers from roles dynamically included via `include_role` tasks (available at runtime only after the `include_role` task executed).
...
Since Ansible 2.14 `meta tasks <ansible.builtin.meta` are allowed to be used and notified as handlers. Note that however `flush_handlers` cannot be used as a handler to prevent unexpected behavior.
