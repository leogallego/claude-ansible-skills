...
\* We added `loop` in Ansible 2.5. as a simpler way to do loops, but we recommend it for most use cases. \* We have not deprecated the use of `with_<lookup>` - that syntax will still be valid for the foreseeable future. \* `loop` and `with_<lookup>` are mutually exclusive. While it is possible to nest them under `until`, this affects each loop iteration.
...
When combining conditionals with a loop, the `when:` statement is processed separately for each item. See the_when_statement for examples.
...
Here, we are iterating over <span class="title-ref">server_configs</span> and printing the key and selected nested fields.
