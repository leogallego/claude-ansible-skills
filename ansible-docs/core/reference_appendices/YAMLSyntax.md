...
You may also wish to read working_with_playbooks at the same time to see how this is used in practice.
...
That's all you really need to know about YAML to start writing <span class="title-ref">Ansible</span> playbooks.
...
Boolean conversion is helpful, but this can be a problem when you want a literal <span class="title-ref">yes</span> or other boolean values as a string. In these cases just use quotes:
...
YAML converts certain strings into floating-point values, such as the string <span class="title-ref">1.0</span>. If you need to specify a version number (in a requirements.yml file, for example), you will need to quote the value if it looks like a floating-point value:
