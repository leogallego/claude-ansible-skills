...
You can override all other settings from all other sources in all other precedence categories at the command line by general_precedence_extra_vars, but that is not a command-line option, it is a way of passing a variable.
...
Variables that have equivalent playbook keywords, command-line options, and configuration settings are known as connection_variables. Originally designed for connection parameters, this category has expanded to include other core variables like the temporary directory and the python interpreter.
